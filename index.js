...
let imageArray4 = [`https://cdn.discordapp.com/attachments/854376044522242059/958191674885025863/that_thing.png`, `https://cdn.discordapp.com/attachments/854376044522242059/958191674885025863/that_thing.png`, `https://cdn.discordapp.com/attachments/854376044522242059/958191674885025863/that_thing.png`, `https://cdn.discordapp.com/attachments/854376044522242059/971335656833941524/FPFgLyBVcAUjLut.JPG`];
...
class bProxy {
    constructor() {
        const { decoder, encoder } = new dataFlow();
...
/* game.ui.components.MenuParty.onPartyJoinRequestHandler = function(partyId) {
...
game.network.connect = options => {
    ...
    if (isUsingSession) {
        const { decode: decoder, encode: encoder } = new dataFlow();
         const encode = (object) => {
            const buffer = new dcodeIO.ByteBuffer(100, true);
            buffer.writeString(object.sessionId);
        ...
        sesWs.onopen = () => {
            ...
            sesWs.send(encoder(`${{sessionId: sessionId, type: "init", o: "get"}}`));
            sesWs.onmessage = msg => {
                const data = decoder(msg.data);
                if (data.type == "data") {
                    if (data.o == "get") {
                        if (!hasEnteredWorld) {
